extends Control

class_name BarContainer


onready var tween: Tween = get_node("Tween")

# Guardando o acesso aos progressBar
onready var health_bar: TextureProgress = get_node("HealthBarBackgroud/HelthBar")
onready var mana_bar: TextureProgress = get_node("ManaBarBackgroud/ManaBar")
onready var exp_bar: TextureProgress = get_node("ExpBarBackgroud/ExpBar")

# Variáveis para os dados correntes
var current_exp: int
var current_mana: int
var current_health: int
	
func init_bar(health: int, mana:int, max_exp_value: int) -> void:
	exp_bar.max_value = max_exp_value
	health_bar.max_value = health
	mana_bar.max_value = mana
	
	# Valores atuais
	health_bar.value = health
	mana_bar.value = mana
	# Já a experiência -> exp_bar sempre começa com ZERO
	exp_bar.value = 0
	
	# Colocando os valores atuais
	current_exp = 0
	current_mana = mana
	current_health = health
	
func increase_max_value(type: String, max_value: int, value: int) -> void:
	match type:
		"Health":
			health_bar.max_value = max_value
			health_bar.value = value
			current_health = value
			
		"Mana":
			mana_bar.max_value = max_value
			mana_bar.value = value
			current_mana = value
			
			
	
	
func update_bar(type: String, value: int) -> void:
	match type:
		"HealthBar":
			call_tween(health_bar, current_health, value)
			current_health = value
			pass
		"ManaBar":
			call_tween(mana_bar, current_mana, value)
			current_mana = value
			pass
		"ExpBar":
			call_tween(exp_bar, current_exp, value)
			current_exp = value
			pass
	
func reset_exp_bar(max_exp: int, value: int) -> void:
	exp_bar.max_value = max_exp
	exp_bar.value = value
	current_exp = value
	# Atualizando a Barra de Experiencia
	call_tween(exp_bar, 0, current_exp)
	
func call_tween(bar: TextureProgress, initial_value: int, final_value: int) -> void:
	# o var _interpólate_value serve apenas para não exibir warnings
	var _interpolate_value: bool = tween.interpolate_property(
		bar,                 # -> Objeto a ser aplicado o efeito
		"value",             # -> Propriedade a ser alterada
		initial_value,       # -> Valor inicial  
		final_value,         # -> Valor final
		0.2,                 # -> Duração
		Tween.TRANS_QUAD,    # -> Aumenta a velocidade na potência de 2
		Tween.EASE_IN_OUT    # -> começa e termina lentamente
		)
	# Começa a execução da animação tween
	# Idem -> para não exibir Warnings (amarelo)
	var _start: bool = tween.start()
	
